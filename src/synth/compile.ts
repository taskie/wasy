...
const compileOscillatorTone = (
	...
	const fixed = source.pitch != null && source.pitch !== "tracking" ? source.pitch.fixed : undefined;
	if (env.type === "adsr") {
		if (def.oneShot) {
...
const isValidDrumNote = (n: number): boolean =>
	Number.isInteger(n) && n >= 0 && n <= 127;
...
export const compileDrumKit = (
	...
	const routingDestination = (voice: DrumVoiceDefinition): AudioNode => {
		switch (voice.routing) {
			case "left": return kit.leftPanpot;
			case "right": return kit.rightPanpot;
			case "center":
			default: return kit.gain;
		}
	};
	...
	if (def.defaultVoice != null) {
		kit.patchMap[0] = compileTone(instrument, def.defaultVoice, routingDestination(def.defaultVoice));
