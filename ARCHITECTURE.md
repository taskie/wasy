...
`SynthEngine.receiveEvent` は `ChannelEvent` をその channel の `Instrument` に送り、`SystemExclusiveEvent` のうち GS Reset (`F0 41 10 42 12 40 00 7F 00 41 F7`) / XG System On (`F0 43 10 4C 00 00 7E 00 F7`) を `matchSysEx` で検出して `SynthEngine.applyResetAll(time)` を呼ぶ (engine 側で 1 度だけ判定するため、Worker による 16 ch 複製で 16 倍の reset が走らない)。`applyResetAll` は全 16 ch に `Instrument.applyReset(time)` を適用した上で、patch を `generatePatch(instrument, 0, isDrumChannel(i, bankMSB))` の既定プログラム (Piano / Standard Kit) に差し替える。`Wasy.load()` / `unload()` も `applyResetAll()` を呼ぶため、曲を切り替えても前の曲のプログラム / コントローラ状態は持ち越されない (`SmfPlayer.unload()` はテンポも SMF 既定の 120 BPM に戻す)。それ以外の SysEx / MetaEvent は従来どおり全 Instrument に broadcast。`matchSysEx` は SMF (`[varlen-length, ...body]`) と Web MIDI (`[...body]`) の両形式を offset 0 と 1 で試して受け入れる。

`Patch` 基底クラスは ADSR エンベロープ (`attackTime` / `decayTime` / `sustainLevel` / `releaseTime`、既定 5 ms / 0 / 1 / 50 ms) と `applyAttack(gainParam, peakGain, time)` / `applyRelease(gainParam, time)` ヘルパを持つ。`SimpleOscillatorPatch` / `NoisePatch` の `onNoteOn` は `applyAttack`、`onNoteOff` は `applyRelease` + `oscillator.stop(time + releaseTime)` の順で release tail を残してから止める。`applyRelease` は `synth/audio-param.ts` の `cancelAndHold(param, time)` でリリース開始値を予約 NoteOff 時点で固定する。同モジュールは `scheduleValueAtTime` / `scheduleLinearRamp` / `cancelScheduled` 経由のイベントを `WeakMap` のシャドウリストに記録し、`cancelAndHold` が `time` 時点の値を線形補間で計算し、`cancelScheduledValues` 後に **`linearRampToValueAtTime(値, time)` のランプアンカー**で固定する。ランプにするのは、`cancelScheduledValues` が `time` を跨ぐ進行中ランプを (イベント時刻 = ランプ終端のため) 丸ごと削除してしまい、`[ディスパッチ時刻, time)` の窓で直前イベントの値 (= ピーク) に跳ね戻るのを防ぐため — 直前イベントから `(time, 値)` への線形ランプは元の減衰直線を正確に再構成する (シンバル expire 時の swell-back 対策)。ネイティブ `cancelAndHoldAtTime` は全ブラウザで意図的に使わない — 仕様上、指定時刻を跨ぐイベントを打ち切るときしかホールドイベントを挿入しないため、持続音の NoteOff (予約イベントが全て過去側) では何も挿入されず、後続の `linearRampToValueAtTime` が attack 終端にアンカーしてリリースが lookahead 分早く始まって聞こえる (Chrome/Safari で可聴、Firefox はそもそも未実装: Bugzilla 1308431)。`param.value` 直読みも使わない (`currentTime` 時点の値のため、先読みと組み合わさると attack 前の 0 にアンカーして短いノートが無音になる)。`GainedOscillatorPatch` / `GainedNoisePatch` 系 (ピアノ系の減衰 envelope と打楽器 one-shot) は自前の gain ramp を `cancelScheduled(time)` で重ねる方式を維持し、`baseGain` は `velocityToGain(event.velocity)` から直接計算する (基底の `applyAttack` が future schedule を組むため `.value` 読みは default 0 を返す)。
...
5. `Wasy` は `TimedEvent` を受けて `engine.receiveEvent(midiEvent, accurateTime)` で発音を Web Audio に予約。`accurateTime` = `currentTime + delayInSeconds (=200ms lookahead) + (eventTick - oldTick) / ticksPerSecond`。lookahead は `SmfPlayer.lookaheadSeconds` (= `Wasy.lookaheadSeconds`) で 50ms〜1s にクランプして変更できる。ディスパッチ済みイベントは旧値のままスケジュールされるため、**変更は停止中に行う**のが契約 (再生中に変えると差分だけ音側のタイムラインがずれる)。なお `onTimedEvent` のディスパッチと `timer.tick` (スケジューリングフロンティア) は可聴音より lookahead 分だけ先行する。「いま聞こえている位置」は `SmfPlayer.audibleTick` (= `Wasy.audibleTick`) が返す — `timer.tick − (lookahead − 経過時間) × tps` を [0, frontier] にクランプした値。`examples/seekable-player` はビュー反映を各イベントの audio time までキューイングし、now ライン / readout も `audibleTick` を使うので、lookahead を変えても表示と音は同期したままになる。ディスパッチが遅れて予約時刻が過去になった場合は `SynthEngine.receiveEvent` が `currentTime` までクランプする — Web Audio に過去時刻のまま渡すとランプ終点だけが過去に残り、release が潰れてクリックになるため (遅延は小さなタイミングスリップに変換される)。
...
- `ControlChange` は以下を直接処理し、それ以外は `patch.receiveEvent` へ委譲:
    ...
    - `1` Modulation Wheel → `setModulation(value, time)` (per-ch LFO depth、`modDepthRangeCents` に対する割合。CC 77 と加算合成)
    - `76` Vibrato Rate → `setVibratoRate(value, time)` (LFO frequency、64=5Hz、±64 で ∓/± 1 オクターブ)
    - `77` Vibrato Depth → `setVibratoDepth(value, time)` (64=変化なし、Mod Wheel と加算)
    ...
    - `120` AllSoundOff (`notePool.unregisterAll(time)` — イベントの audio time で即時消音、release tail なし)
    ...
    - `122` Local Control → no-op (soft synth にローカル鍵盤はない)
    - `123` AllNotesOff / `124–127` Omni Off/On・Mono/Poly On → `allNotesOff(time)`。全発音ノートに通常 NoteOff を送り (release tail を残す) sustain も解除。CC 120 と異なり即時消音はしない。Mono/Poly モード自体の切り替えは未対応 (暗黙の All Notes Off のみ honor)
